...
function sN(dl){setTimeout(()=>{if(st!=='ended')return;setS('offline',false);aS('❌ '+C.n+" didn't come back online. Start a new chat 💫",true);st='gone';dI();sv();},dl);}
async function send(){if(busy)return;const t=inp.value.trim();if(!t||st==='ended'||st==='gone'||st==='ghosting')return;inp.value='';ap(t,'s');um++;busy=true;dI();if(um>=ga){busy=false;tG(t);return;}try{setS('typing...',true);const rp=await api(t);sT();await wt(tD(rp));hT();ap(rp,'r');setS('Online',true);}catch(e){hT();setS('Online',true);}busy=false;eI();inp.focus();}
inp.addEventListener('keydown',e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();send();}});
(async()=>{const saved=ld();if(saved&&saved.m&&saved.m.length>0){saved.m.forEach(m=>dom(m));st=saved.s||'active';um=saved.m.filter(m=>m.t==='s').length;if(st==='ended'){dI();if(dR())sR(rD());else sN(3e4+Math.random()*45e3);}else if(st==='gone'){dI();}else{eI();ga=um+pG();}}else{setS('typing...',true);sT();const g=await api('hi');await wt(tD(g));hT();ap(g,'r');setS('Online',true);eI();}inp.focus();})();
</script></body></html>"""
